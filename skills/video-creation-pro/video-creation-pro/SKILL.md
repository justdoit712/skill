...
description: 电商运营与自媒体创作者在为美妆、家居、智能硬件等商品制作短视频时，请使用此技能。10大AI智能体协同，从卖点提炼、分镜生图到音效字幕，一键调用COZE API自动合成高质量商品视频，彻底告别本地代码依赖，轻松实现从创作、质检到数据迭代的全流程闭环。
dependency:
  python: []
  system: []
---
...
1. **核心执行步骤**（强制按此执行，杜绝本地操作）：
   ...
   - API参数配置：在COZE视频大模型API中按以下参数**精准配置**，无任何修改
   ...
   - 合成执行：启动API合成，开启**合成状态实时回调**，若出现合成异常，立即触发**API重试机制**（重试次数≤3次，超时时间10秒），不进入本地死循环，重试失败则反馈技能异常
...
**角色定位**：技能迭代数据分析师，核心职责是收集、统计、分析全流程数据（重点含**本地依赖包违规次数、API适配不合格次数、字幕适配问题次数**），输出迭代建议，优化智能体提示词与协同逻辑，提升技能整体表现
...
- 参考文档：
  - 见 [references/usage_limit.md](references/usage_limit.md)（使用限制与超限处理机制）
  - 见 [references/subtitle_format.md](references/subtitle_format.md)（字幕格式规范，COZE API适配）
  - 见 [references/audio_format.md](references/audio_format.md)（音频格式规范，COZE API适配）
  - 见 [references/storyboard_format.md](references/storyboard_format.md)（分镜脚本格式）
  - 见 [references/use_cases.md](references/use_cases.md)（使用案例）
  - 见 [references/agent_specs.md](references/agent_specs.md)（10大智能体完整开发规范）
