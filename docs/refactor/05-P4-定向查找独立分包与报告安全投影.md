...
本步骤的核心是将此前已在 P1 完成行为修复的定向查找逻辑，正式独立为顶层业务包 **`src/finder/`**，并解决展示层的安全合规问题：
1. **彻底与主目录解耦**：落实“定向查找跳过目录规则”的强制约束，`finder` 业务包在物理上**绝对不导入** `catalog` 的任何代码，运行时零目录副作用。
2. **拆解编排大文件**：将 729 行的 `skill_finder.py` 与 589 行的 `find_evaluate.py` 拆分为内聚的子模块（配置、搜索、规划、评估、证据核验、运行编排、报表）。
3. **建立安全白名单数据投影**：严禁直接将本地运行的原始 JSON 粗暴复制到前端公共目录。建立安全投影转换器，彻底剔除本机绝对路径、模型配置细节与异常堆栈。
...
from src.shared.models import DocumentSnapshot
...
实现严密的三计数器控制循环，将 F1～F5 的修复逻辑正式固化：
...
## 4. 报告安全投影机制（`finder/report.py`）

### 4.1 痛点诊断
此前代码直接将包含本机环境路径（如 `D:/Code/opensource/skill/data/local/...`）的 JSON 写入 `public/data/find-report.json`，存在信息泄露风险，且前端容易受到脏数据污染。
...
flowchart TD
    ...
    RunState --> Projection[3. 安全投影过滤器 build_public_projection]
    Projection --> PublicJson[4. 前端脱敏产物<br>public/data/find-report.json]
```

### 4.3 白名单安全投影实现规范
前端页面只允许接收经过白名单过滤的脱敏字段：
...
* **原子写保护**：向 `public/data/find-report.json` 写入时，必须使用 `write_json_atomic`，杜绝前端并发拉取到残缺 JSON。
