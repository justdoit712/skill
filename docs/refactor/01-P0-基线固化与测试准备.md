...
### 3.1 现状与基线测试状态
* **测试套件基线**：当前仓库 `tests/` 下共 15 个测试文件，基线测试用例共计 **235 项**，执行 `python -m unittest discover -s tests` 处于全部通过（235 tests OK）状态。
* **探针数据自包含化要求**：此前离线评估报告中引用的三处外部机器路径（`architecture_probes.py` 等）仅供离线参考，测试套件绝对不得依赖开发者个人机器的外部文件。
* **测试样本沉淀**：P0 阶段将离线探针沉淀出的典型边界样本（假引文、截断 Markdown、空响应、跨行伪造）转存至 `tests/fixtures/finder_samples/` 目录下，作为后续 P1 自动化测试的自包含夹具。
