...
删除阶段结束时 **git 跟踪 21 个文件**，此前为 996 个。（`.idea/` 下 10 个文件属本机 IDE 配置，未被 git 跟踪且已忽略，不计入；工作区文件数因此比跟踪数多。）已确认的不可逆项：`projects/assistant/assets/logo/` 下 13 个被忽略文件共 137.2 MB（§1.2）已按决定删除，无法从 git 恢复。
