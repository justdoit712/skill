...
状态：代码改动与离线回归已实施；真实模型、远端 Actions 和部署验收未执行。编制日期：2026-09-22；深化修订日期：2026-09-23。当前结果见 [修复验收记录](./audit/2026-09-23-分层重构修复验收.md)。

依据：[src 分层与轻量化评估](./src分层与轻量化评估.md)。最新核对基准为提交 `add3843`，实施时还须重新确认工作区改动，不能只按提交版本回退或覆盖本地修改。原方案基准 `361cd08` 仅作历史记录。

本方案将代码评估转化为分批改动、接口约定、兼容要求和验收清单。以下保留编制时的设计要求；历史基准、行号和测试缺失描述不代表修复后的工作区现状。

**深化修订**：依据评估第 10～15 节补充材料集合身份、存储所有权与故障恢复要求。本地已有查找报告修改须保留。先保留旧 ID/JSON 兼容，再按批次改造，不能借重构批量触发重评。
...
上述命令已在本轮修复后执行（Python 使用现有 `.venv`）；新增生产入口测试位于 `tests/test_refactor_acceptance.py`。具体覆盖与未执行的外部验收见修复验收记录，不将测试数量等同于全部场景证明。
