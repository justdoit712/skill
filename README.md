...
普通查找默认启用**质量优先筛选（规则 1.1.0）**：六项基础检查之外，还要求实际价值、可执行性、结果验证三项质量检查通过；所有通过项的原文引用由程序核对完整行与行号。拟推荐条目再进行一次不接收初评结论的独立模型评估，两轮都通过且分类一致才进入**推荐区**。存在质量缺口、无法核验的引用或两轮分歧时留在**候选区**并注明原因。

初评不通过通常只调用一次，拟推荐条目通常调用两次（不含重试），并可能需要评估更多候选才能新增 50 项。每次请求分别计入原有 Token 总预算；初评后预算耗尽或用量缺失时保存初评、停止本轮，下次运行只继续复核。卡片中的“筛选依据”展示三项理由和复核状态。旧条目不会自动冒充已通过新规则；已完成的本地候选池条目不会因规则升级自动重评。完整说明见 [运行说明](docs/运行说明.md)。
...
| [已收录管理实施方案](docs/plans/2026-09-24-已收录管理实施方案.md) | 已收录管理规范：已收录隐藏、身份匹配、隐私隔离、同步变更包与验收用例 |
| [全量审计报告](docs/audit/2026-09-23-全量审计报告.md) | 按入口的健康度、P0–P3 缺陷（含位置与复现）、覆盖率实测、文档漂移与修复顺序 |
| [架构约定](docs/架构约定.md) | 分层依赖、材料与状态契约、保存和恢复约束 |
| [分层重构修复验收](docs/audit/2026-09-23-分层重构修复验收.md) | 本轮修复、验证证据、离线恢复命令与验证边界 |
| [已收录功能代码复核与修复方案](docs/audit/2026-09-24-已收录功能代码复核与修复方案.md) | 新功能的五项待修问题、复现、修复位置与验收标准 |
